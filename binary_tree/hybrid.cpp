...
int readFileandSetTree(string argv, vector<BinaryTree *> &vectorTree, int &last_id)
{
    ...
    if (infile.is_open())
    {
        ...
        {
            index = id / 5000;
            if (index > lastTreeIndex)
            {
                for (int i = lastTreeIndex; i < index; i++)
                {
                    vectorTree.insert(vectorTree.end(), new BinaryTree);
                }
                lastTreeIndex = index;
            }
            ...
            vectorTree[index]->insert(newEmployee);
            ++last_id;
        };
    }
    else
...
{
    vector<BinaryTree *> vectorTree;
    int last_id = 0;
    int indexOfVector = 0;
    string line, line_info;
    string type, req_id, new_sal, new_dep;
    last_id = readFileandSetTree(argv[1], vectorTree, last_id);
    ...
    while (getline(read, line))
    {
        ...
        if (type == "ADD")
        {
            ...
            indexOfVector = last_id / 5000;
            if (indexOfVector >= int(vectorTree.size()))
            {
                for (int i = int(vectorTree.size()) - 1; i < indexOfVector; i++)
                {
                    vectorTree.insert(vectorTree.end(), new BinaryTree);
                }
            }
            vectorTree[indexOfVector]->insert(newEmployee);
        }
        else if (type == "UPDATE")
        {
            ...
            getline(line_info, new_sal, ';');
            getline(line_info, new_dep);
            indexOfVector = stoi(req_id) / 5000;
            if (indexOfVector < int(vectorTree.size()))
            {
                vectorTree[indexOfVector]->update(stoi(req_id), stoi(new_sal), stoi(new_dep));
            }
            else
            {
                cout << "ERROR: An invalid ID to update\n";
        ...
        else if (type == "DELETE")
        {
            ...
            if (indexOfVector < int(vectorTree.size()))
            {
                vectorTree[indexOfVector]->remove(stoi(req_id));
            }
            else
            {
                cout << "ERROR: An invalid ID to delete\n";
    ...
    for (auto i = vectorTree.begin(); i != vectorTree.end(); i++)
    {
        (*i)->printToFile(file);
        (*i)->deleteTree();
        delete (*i);
